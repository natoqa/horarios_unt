...
export function Sidebar({ collapsed, onToggle, mobileOpen, onMobileClose }: SidebarProps) {
  ...
  const NavItem = ({ item, isCollapsed }: { item: typeof menuPrincipal[0]; isCollapsed: boolean }) => {
    ...
    return (
      ...
      >
        ...
        {isCollapsed && (
          <div className="absolute left-full ml-2 hidden rounded-md bg-gray-900 dark:bg-gray-700 px-2.5 py-1.5 text-xs font-medium text-white shadow-lg group-hover:block whitespace-nowrap z-50">
            {item.label}
          </div>
        )}
      </Link>
  ...
  const buildContent = (isCollapsed: boolean) => (
    <div className="flex h-full flex-col overflow-hidden">
      {/* Header */}
      <div className="relative h-16 shrink-0 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center h-full px-4 gap-3">
          ...
          <div className={cn(
            ...
            <p className="text-sm font-bold text-gray-900 dark:text-gray-100 leading-tight">Horarios UNT</p>
      ...
      {/* Footer - Usuario */}
      <div className={cn('border-t border-gray-100 dark:border-gray-800 shrink-0 relative', isCollapsed ? 'p-2' : 'p-3')} ref={userMenuRef}>
        {!isCollapsed ? (
          <div className="flex items-center gap-3 rounded-lg p-2.5 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">{nombre}</p>
          ...
          <div className="flex justify-center">
            <button
              ...
              className={cn(
                ...
                userMenuOpen && 'ring-2 ring-primary-300 ring-offset-2 dark:ring-offset-gray-900'
              )}
            >
              <span className="text-[11px] font-bold text-white">{iniciales}</span>
            </button>
            ...
            {userMenuOpen && (
              <div className="absolute left-full bottom-2 ml-2 w-56 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-xl z-50 overflow-hidden">
                ...
                <div className="py-1.5">
                  <Link
                    ...
                    className="flex items-center gap-2.5 px-4 py-2 text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-gray-100 transition-colors"
                  >
                ...
                <div className="border-t border-gray-100 dark:border-gray-700 py-1.5">
                  <button
                    onClick={() => signOut({ callbackUrl: '/login' })}
                    className="flex items-center gap-2.5 px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors w-full"
                  >
                    <LogOut className="h-4 w-4" />
                    Cerrar sesión
                  </button>
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed inset-y-0 left-0 z-50 w-[272px] bg-white dark:bg-gray-900 shadow-2xl transition-transform duration-300 ease-in-out lg:hidden',
      ...
      >
        <aside className="h-full bg-white dark:bg-gray-900 border-r border-gray-200/80 dark:border-gray-700/80 overflow-hidden">
          {buildContent(collapsed)}
        </aside>
        <button
          onClick={onToggle}
          className="absolute top-7 -right-3 w-6 h-6 flex items-center justify-center rounded-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:border-gray-300 dark:hover:border-gray-600 shadow-sm transition-colors z-50"
